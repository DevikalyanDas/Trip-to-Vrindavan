...
MatLib* gettable2() {
	...
	ImageTexture* woodtex = new ImageTexture("models/tex_wood.png");
	ConstantTexture* blacktex = new ConstantTexture(RGBColor::rep(0.0f));
	matlib->insert(std::pair<std::string, Material*>("Default_OBJ.003", new LambertianMaterial(blacktex, blacktex)));
	matlib->insert(std::pair<std::string, Material*>("Default_OBJ.002", new LambertianMaterial(blacktex, blacktex)));
	matlib->insert(std::pair<std::string, Material*>("Default_OBJ.001", new LambertianMaterial(blacktex, blacktex)));
	matlib->insert(std::pair<std::string, Material*>("Default_OBJ", new LambertianMaterial(blacktex, blacktex)));
	matlib->insert(std::pair<std::string, Material*>("Material.002", new LambertianMaterial(grasstex, blacktex)));
	matlib->insert(std::pair<std::string, Material*>("Material.003", new LambertianMaterial(grasstex, blacktex)));
	//matlib->insert(std::pair<std::string, Material*>("Material.001", new GlassMaterial(2.0f)));
	matlib->insert(std::pair<std::string, Material*>("Material.005", new LambertianMaterial(woodtex, blacktex)));
...
void a_indexing() {
	...
	MatLib* gettable1 = gettable2();
	loadOBJ(scene, "models/", "scene_nobase.obj", gettable1);
